<script setup>
	 ...
	 * @since 1.0.0
	 */
	const enablePlugin = "enable_product_comparison";
	const enableStyles = "load_styles_from_plugin";
	...
	function optionVisibility(key, dependent) {
		switch (currentPage) {
			case "settings":
				return key === enablePlugin
					? true
					: optionDependency(dependent);
			case "design":
				return key === enableStyles
					? true
					: optionDependency(dependent);
			default:
				return false;
